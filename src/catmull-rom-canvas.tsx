...
import { cubicCatmullRomSpline } from './catmull-rom';
const vs_src = `
...
`
function findFocus(x: number, y: number, controlPoints: readonly vec3[]) {
    for (let i = 0; i < controlPoints.length; i++) {
        if (vec3.sqrDist(controlPoints[i], [x, y, 0]) < 0.01) return i;
    }
    return null;
}
type SampleResult = { uniformResult: vec3[], chordalResult: vec3[], centripetalResult: vec3[] };
function fillVertices(sampleResult: SampleResult, controlPoints: readonly vec3[]) {
    const length = sampleResult.uniformResult.length + sampleResult.chordalResult.length + sampleResult.centripetalResult.length + controlPoints.length;
    const vertices = new Float32Array(length * 3);
    let j = 0;
    for (let i = 0; i < sampleResult.uniformResult.length; i++, j++) vertices.set(sampleResult.uniformResult[i], j * 3);
    for (let i = 0; i < sampleResult.chordalResult.length; i++, j++) vertices.set(sampleResult.chordalResult[i], j * 3);
    for (let i = 0; i < sampleResult.centripetalResult.length; i++, j++) vertices.set(sampleResult.centripetalResult[i], j * 3);
    for (let i = 0; i < controlPoints.length; i++, j++) vertices.set(controlPoints[i], j * 3);
    return vertices;
}
function fillColors(sampleResult: SampleResult, controlPoints: readonly vec3[]) {
    const length = sampleResult.uniformResult.length + sampleResult.chordalResult.length + sampleResult.centripetalResult.length + controlPoints.length;
    const colors = new Float32Array(length * 3);
    let j = 0;
    for (let i = 0; i < sampleResult.uniformResult.length; i++, j++) colors.set([1, 0.6, 0.6], j * 3);
    for (let i = 0; i < sampleResult.chordalResult.length; i++, j++) colors.set([0.6, 1, 0.6], j * 3);
    for (let i = 0; i < sampleResult.centripetalResult.length; i++, j++) colors.set([0.6, 0.6, 1], j * 3);
    for (let i = 0; i < controlPoints.length; i++, j++) colors.set([0.2, 0.5, 1], j * 3);
    return colors;
}
function fillSizes(sampleResult: SampleResult, controlPoints: readonly vec3[], focus: number | null) {
    const length = sampleResult.uniformResult.length + sampleResult.chordalResult.length + sampleResult.centripetalResult.length + controlPoints.length;
    const sizes = new Float32Array(length);
    sizes.fill(5);
    if (focus !== null) sizes[length - controlPoints.length + focus] = 10;
    return sizes;
}
...
function draw(
    ...
    samplePoints: { uniformResult: vec3[], chordalResult: vec3[], centripetalResult: vec3[]},
    focus: number | null,
    config: CubicCatmullRomSpline2DConfig,
) {
    ...
    const samplePointsLength = samplePoints.uniformResult.length + samplePoints.chordalResult.length + samplePoints.centripetalResult.length;
    if(config.knotParametrization.uniform) 
        gl.drawArrays(gl.LINE_STRIP, 0, samplePoints.uniformResult.length);
    if(config.knotParametrization.chordal) 
        gl.drawArrays(gl.LINE_STRIP, samplePoints.uniformResult.length, samplePoints.chordalResult.length);
    if(config.knotParametrization.centripetal) 
        gl.drawArrays(gl.LINE_STRIP, samplePointsLength - samplePoints.centripetalResult.length, samplePoints.centripetalResult.length);
    if (config.showSamplePoints) {
        gl.drawArrays(gl.POINTS, 0, samplePointsLength);
    }
    if (config.showControlPoints) {
        gl.drawArrays(gl.LINE_STRIP, samplePointsLength, config.controlPoints.length);
        gl.drawArrays(gl.POINTS, samplePointsLength, config.controlPoints.length);
    }
...
export type CubicCatmullRomSpline2DConfig = {
    readonly tag: 'catmull-rom',
    readonly showControlPoints: boolean,
    readonly showSamplePoints: boolean,
    readonly controlPoints: readonly vec3[],
    readonly setControlPoints: React.Dispatch<React.SetStateAction<readonly vec3[]>>,
    readonly knotParametrization: {
        readonly uniform: boolean,
        readonly chordal: boolean, 
        readonly centripetal: boolean,
    },
};
...
export function CubicCatmullRomCurveCanvas({ config, canvasSize }: { config: CubicCatmullRomSpline2DConfig, canvasSize: readonly [number, number] }) {
    ...
    React.useEffect(() => {
        ...
        switch (cursor.cursorStyle) {
            ...
            case 'move':
                ...
                if (leftClicked) {
                    const newControlPoints = Array.from(config.controlPoints);
            ...
            case 'grabbing':
                ...
                if (leftClicked) {
                    ...
                    config.setControlPoints(config.controlPoints.map(p => vec3.add(vec3.create(), p, translation)));
                    setCursor({ cursorStyle: 'grab' });
                }
                break;
        }
    ...
    React.useEffect(() => {
        ...
        const samplePoints = cubicCatmullRomSpline(config.controlPoints, config.knotParametrization);
        draw(gl, attributes, uniforms, translation, samplePoints, focus, config);
    }, [gl, attributes, uniforms, translation, focus, config, config.controlPoints, config.showControlPoints, config.showSamplePoints, config.knotParametrization]);
